...
function drawConnect(connect) {
    context.save();
    if (connect instanceof StraightConnect)
        drawStraightConnect(connect);
...
function getCenter(coordinate) {
    return ((TILE_SIZE + TILE_SPACE) * (coordinate + 1) + TILE_SIZE / 2 + TILE_SPACE / 2);
}
